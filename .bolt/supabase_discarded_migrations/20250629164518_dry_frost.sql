/*
  # Fix auth.users access in functions
  
  1. Problem
    - Functions trying to directly access auth.users table are failing
    - Error: "must be owner of relation users"
    - Need to use auth.users() function instead of direct table access
  
  2. Solution
    - Update all functions that access auth.users to use auth.users() function
    - Fix get_user_id_by_email and get_user_email_by_id functions
    - Update dependent functions to use these helper functions
*/

-- Fix get_user_id_by_email function to use auth.users() function
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Error in get_user_id_by_email: %', SQLERRM;
    RETURN NULL;
END;
$$;

-- Fix get_user_email_by_id function to use auth.users() function
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Error in get_user_email_by_id: %', SQLERRM;
    RETURN 'Error: ' || SQLERRM;
END;
$$;

-- Fix check_email_admin_status function
...
-- Fix delete_user_safely function
...
COMMENT ON FUNCTION public.get_user_id_by_email(text) IS 'Securely get user ID by email using auth.users() function';
COMMENT ON FUNCTION public.get_user_email_by_id(uuid) IS 'Securely get user email by ID using auth.users() function';
COMMENT ON FUNCTION public.check_email_admin_status(text) IS 'Check if a specific email has admin role - fixed auth.users access';
COMMENT ON FUNCTION public.list_admin_users() IS 'List all users with admin role - fixed auth.users access';
COMMENT ON FUNCTION public.get_all_users_with_roles() IS 'Admin-only function to get all users with their roles - fixed auth.users access';
COMMENT ON FUNCTION public.make_user_admin(text) IS 'Function to grant admin role to a user by email - fixed auth.users access';
COMMENT ON FUNCTION public.delete_user_safely(uuid, text) IS 'Admin-only function to safely delete a user - fixed auth.users access';