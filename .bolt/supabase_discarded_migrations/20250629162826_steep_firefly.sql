/*
  # Fix duplicate keys in user_roles table

  1. Problem
    - Unique constraint violation when trying to add UNIQUE (user_id, role)
    - Duplicate entries exist in the user_roles table
    - Need to identify and remove duplicates before adding constraint

  2. Solution
    - Identify duplicate (user_id, role) combinations
    - Keep only one record for each combination (the oldest one)
    - Then safely add the unique constraint
*/
...
SELECT 
  ...
  role, 
  COUNT(*) as count,
  MIN(id) as keep_id -- We'll keep the record with the minimum id (oldest)
